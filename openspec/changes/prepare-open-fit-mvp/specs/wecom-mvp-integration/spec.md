...
系统 SHALL 面向员工暴露两个企业微信智能机器人入口：`Open Fit 打卡助手` 与 `Open Fit AI 教练`。打卡助手 MUST 只处理打卡、补图、确认提交和打卡状态；AI 教练 MUST 处理训练建议、活动规则、活动信息和排行榜查询，不得创建或提交打卡记录。

#### Scenario: 群内 @ 机器人提交今日运动

- **WHEN** 员工在企业微信群中 @ Open Fit 打卡助手并发送包含文字和图片的打卡内容
- **THEN** 系统必须识别为打卡意图，生成待确认打卡结果，关联图片附件，并回复结构化摘要与确认指令
...
- **THEN** 系统必须调用 AI 图片识别生成待确认打卡并要求员工确认，识别失败时提示补充文字

#### Scenario: 群内 @ 机器人确认提交

- **WHEN** 员工在打卡助手待确认状态下回复“确认”或“提交”
- **THEN** 系统必须将最近一条待确认打卡提交为有效打卡，并回复提交成功摘要
