...
第一阶段 SHALL 不把真实企业微信 OAuth、自建应用个人消息、真实 AI 模型调用、图片识别、完整排行榜规则和生产级活动配置作为阻塞项；这些能力必须被记录为第二阶段或第三阶段工作。

#### Scenario: 验收第一阶段

- **WHEN** 第一阶段准备验收
- **THEN** 验收标准必须聚焦可运行框架、可体验闭环、企业微信触达测试和工程边界，不得因图片识别或完整 OAuth 未完成而判定失败
