...
- [x] 15.3 企业微信机器人首次识别未知 `userid` 时，在配置 `WECOM_CORP_ID` 和 `WECOM_APP_SECRET` 后调用企业微信通讯录接口补全姓名和部门；未配置或失败时降级为 userid 档案。
...
- [x] 1.3 确认第一阶段必须能接入企业微信智能机器人 SDK 长连接；后台群提醒通过打卡助手长连接主动推送，自建应用消息、OAuth 和成员同步只做配置预检。
...
- [x] 5.7 实现企业微信配置体验：展示智能机器人长连接群推送说明、测试发送结果和自建应用配置状态。
...
- [x] 17.2 Web 新增群管理页面，支持新增群、展示绑定口令、选择当前群和粘贴中文名导入成员。
- [x] 17.3 企业微信打卡助手入站消息支持识别绑定口令，保存当前群 `chatid`，并观察群内发言成员。
- [x] 17.4 后端支持通过企业微信通讯录接口按中文名匹配 userid，并将匹配成功成员写入当前群。
