...
- 企业微信接入层：智能机器人 SDK 长连接、群内 @、按群主动推送、通讯录查询。
...
- Web 群管理负责创建待绑定群、展示绑定口令、保存当前群、导入群成员；运营看板、成员管理、活动配置和排行榜都以当前群为数据边界。
- 企业微信通讯录接口用于把中文名匹配到 userid，以及补全首次入站成员的姓名和部门；项目不再提供面向用户的企业微信配置页面。
- Bot Secret、应用 secret、access token 不得提交到仓库。
...
- 群管理：创建群、绑定企业微信 chatid、选择当前群、通过中文名导入群成员。
