...
flowchart TB
  ...
  subgraph ApiLayer["API 模块化单体层"]
    ...
    OpsModule["activities / dashboards / reminders / leaderboards\n活动规则 / 运营统计 / 未打卡提醒 / 排行榜"]
  end
...
- 打卡助手只负责打卡闭环和群绑定；AI 教练不创建打卡记录。
- 企业微信身份以 `from.userid` 为主键来源；中文名和部门来自 Excel 名册导入。
- 群内主动推送以 `WeComGroup.chatId` 为目标，通过打卡助手长连接发送。
...
flowchart TD
  ...
  CreateGroup --> BindInWeCom["目标群 @ 打卡助手发送绑定口令"]
