...
- 企业微信入站消息需要归一化为文本、图片、图文混排和引用消息四类业务输入；图片素材下载或占位保存由企业微信适配层完成，打卡业务只依赖受控附件元数据。
- 企业微信引用消息来自 SDK 消息体的 `body.quote`。AI 教练收到引用消息时，业务层必须把引用内容作为本轮优先上下文传给模型，并降低短期历史会话优先级，避免基于旧话题回答。
...
- 企业微信 AI 教练入站消息处理流程为：解析 `userid/chatid` -> 输入安全围栏 -> 按是否存在引用消息组装上下文 -> 调用 OpenAI-compatible `messages[]` -> 输出安全围栏 -> 保存通过校验的问答 -> 回复企业微信。
- 无引用消息时，AI 教练注入短期会话摘要和最近 10 条消息；有引用消息时，AI 教练优先注入结构化引用内容和当前问题，短期历史不进入最近消息窗口，长期结构化画像仍可作为弱背景。
